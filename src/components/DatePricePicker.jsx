...
const UserIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 text-gray-400">
    {/* MODIFIED: Corrected the SVG path data */}
...
export default function DatePricePicker({ onDateChange = () => {} }) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (isPast) {
        ...
        if (isCheckIn || isCheckOut) {
          ...
          priceTextClass += " text-indigo-200";
        } else if (isInRange) {
          classes += " bg-indigo-100 text-indigo-700";
          priceTextClass += " text-green-800";
        } else if (isToday) {
          classes += " bg-gray-200 text-gray-800";
          priceTextClass += " text-green-700";
        } else {
    ...
    return (
      <div className="absolute top-20 mt-2 w-[95vw] max-w-lg left-1/2 -translate-x-1/2 md:w-auto md:max-w-none md:left-0 md:translate-x-0 bg-white p-4 rounded-lg shadow-2xl border border-gray-200 z-50">
        ...
        <div className="mt-4 flex justify-end">
          <button onClick={() => { setCheckInDate(null); setCheckOutDate(null); }} className="text-sm font-semibold text-gray-600 hover:text-gray-900 px-4 py-2 rounded-md hover:bg-gray-100">
            Clear Dates
          </button>
        </div>
      </div>
  ...
  return (
    <div className="w-full flex items-start justify-center px-4 sm:px-1 font-sans">
      <div ref={pickerRef} className="relative w-full max-w-4xl">
        <div className="bg-white rounded-xl md:rounded-full shadow-lg p-2 flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-1 w-full">
          ...
          <div className='flex-1 w-full flex flex-row items-center'>
            {/* Duration */}
            {numberOfNights > 0 && (
              <div className="flex-1 w-full p-2 text-left">
            ...
            {/* Guests */}
            <div 
              onClick={() => { setIsGuestPickerOpen(prev => !prev); setIsCalendarOpen(false); }}
              className="flex-1 w-full p-2 relative text-left cursor-pointer rounded-full hover:bg-gray-100"
            >
          ...
          {/* Search Button */}
          <button className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-4 sm:px-6 rounded-full transition duration-300 w-full md:w-auto">
            Search
          </button>
        </div>
